...
export default class AccountPage extends Component {
    ...
    if (this.state.customerId && new Date(this.state.expiresAt) > currTime) {
      ...
      return (
        <div>
          ...
          <p>
            It will expire {date.format(expired, 'MMM DD YYYY')}
          </p>
          <div className='subscription-stripe'>
            <Stripe/>
          </div>
        </div>
      );
    } else {
      return (<span className="premium">
          <p>
            Go Premium! 
          </p>
          <Stripe />
  ...
  render() {
    ...
    return (
      <div>
        <ProfileComponent state={this.state} message={ userId && this.checkSubscription()}/>
